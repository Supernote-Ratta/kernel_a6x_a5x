...
static const match_table_t exfat_tokens = {
    ...
    { Opt_utf8,           "utf8" }, /* empty or 1 or yes or true */
    { Opt_utf8,           "utf8=1" },
    { Opt_utf8,           "utf8=yes" },
    { Opt_utf8,           "utf8=true" },
	{ Opt_debug,          "debug" },
	{ Opt_namecase,       "namecase" },
	{ Opt_codepage,       "codepage" },
	{}
};

static int exfat_parse_options(struct super_block *sb,
							   char *options,
	...
	while ((p = strsep(&options, ",")) != NULL) {
		...
		case Opt_uid:
			if (match_int(&args[0], &opt))
				return -EINVAL;
			opts->fs_uid = make_kuid(current_user_ns(), opt);
		...
		case Opt_gid:
			if (match_int(&args[0], &opt))
				return -EINVAL;
			opts->fs_gid = make_kgid(current_user_ns(), opt);
		...
		case Opt_umask:
			if (match_octal(&args[0], &opt))
				return -EINVAL;
			opts->fs_fmask = opts->fs_dmask = opt;
			break;
		case Opt_dmask:
			if (match_octal(&args[0], &opt))
				return -EINVAL;
		...
		case Opt_allow_utime:
			if (match_octal(&args[0], &opt))
				return -EINVAL;
			opts->allow_utime = opt & (S_IWGRP | S_IWOTH);
			break;
		case Opt_charset:
		...
		case Opt_time_offset:
			if (match_int(&args[0], &opt))
				return -EINVAL;
			if (opt < -12 * 60 || opt > 12 * 60)
				return -EINVAL;
			opts->time_offset = opt;
			break;
		case Opt_utf8:
		case Opt_debug:
		case Opt_namecase:
...
static int exfat_setup(struct super_block *sb)
{
	int ret;
	struct exfat_sb_info *sbi = EXFAT_SB(sb);
...
static int exfat_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	exfat_parse_options(sb, data, opts);
...
static struct dentry *exfat_mount(struct file_system_type *fs_type,
