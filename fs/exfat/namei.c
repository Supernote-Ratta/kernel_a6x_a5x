...
static int exfat_d_revalidate(struct dentry *dentry, unsigned int flags)
{
	...
	ret = d_inode(dentry->d_parent)->i_version == exfat_d_version(dentry);
...
static int exfat_d_cmp(const struct dentry *parent,
		...
		bool excl)
{
	...
	err = exfat_add_entry(dir, dentry->d_name.name, &cdir, TYPE_FILE, &info);
	...
	inode->i_mtime = inode->i_atime = inode->i_ctime =
		EXFAT_I(inode)->i_crtime = current_fs_time(inode->i_sb);
...
static int exfat_unlink(struct inode *dir, struct dentry *dentry)
{
	...
	inode_inc_iversion(dir);
	dir->i_mtime = dir->i_atime = current_fs_time(dir->i_sb);
		...
		struct inode *new_dir, struct dentry *new_dentry)
{
	...
	new_dir->i_ctime = new_dir->i_mtime = new_dir->i_atime =
		EXFAT_I(new_dir)->i_crtime = current_fs_time(new_dir->i_sb);
	...
	if (new_inode) {
		...
		new_inode->i_ctime = EXFAT_I(new_inode)->i_crtime =
			current_fs_time(new_inode->i_sb);
	}
