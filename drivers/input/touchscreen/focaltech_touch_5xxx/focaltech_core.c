...
void fts_init(void)
{
    ...
    //ret = fts_write_reg(0x80, 0xff);
    //if (ret < 0) {
    //    FTS_ERROR("hid2std cmd write fail");
    //} else {
    //    msleep(10);
    ret = fts_read_reg(0x80, &value);
    if (ret < 0) {
        FTS_ERROR("hid2std cmd read fail");
    } else {
        FTS_DEBUG("fts_init0x80=%x", value);
    }
    //}
}
...
static int fts_input_report_key(struct fts_ts_data *data, int index)
{
    ...
    for (i = 0; i < data->pdata->key_number; i++) {
        if ((x >= x_dim[i] - FTS_KEY_DIM) && (x <= x_dim[i] + FTS_KEY_DIM) && (y >= y_dim[i] - FTS_KEY_DIM) && (y <= y_dim[i] + FTS_KEY_DIM)) {
            if (EVENT_DOWN(data->events[index].flag) && !(data->key_state & (1 << i))) {
...
static int fts_input_report_b(struct fts_ts_data *data)
{
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            x = events[i].x;
            y = events[i].y;
            if (data->pdata->exchange_x_y_flag) {
                int t = x;
                x = y;
                y = t;
            }
            if (data->pdata->revert_x_flag) {
                x = data->pdata->x_max - x;
            }
            if (data->pdata->revert_y_flag) {
                y = data->pdata->y_max - y;
            }
            //ratta_mt_record(events[i].id, x, y, true);
            ...
            if ((data->log_level >= 2) ||
                ...
                FTS_DEBUG("[B]P%d(%d, %d)[p:%d,tm:%d] DOWN!", events[i].id, x, y, events[i].p, events[i].area);
            }
        } else {
            uppoint++;
            input_mt_report_slot_state(data->input_dev, MT_TOOL_FINGER, false);
            data->touchs &= ~BIT(events[i].id);
            //ratta_mt_record(events[i].id, 0, 0, false);
            if (data->log_level >= 1) {
    ...
    for (i = 0; i < data->touch_point; i++) {
        ...
        if (EVENT_DOWN(events[i].flag)) {
            ...
            if ((data->log_level >= 2) || ((1 == data->log_level) && (FTS_TOUCH_DOWN == events[i].flag))) {
                FTS_DEBUG("[A]P%d(%d, %d)[p:%d,tm:%d] DOWN!", events[i].id, x, y, events[i].p, events[i].area);
            }
            touchs++;
        }
    }
...
static int fts_input_pen_report(struct fts_ts_data *data)
{
    ...
    if (data->log_level >= 2  ||
        ...
        FTS_DEBUG("[PEN]x:%d,y:%d,p:%d,inrange:%d,tip:%d,flag:%d DOWN", pevt->x, pevt->y, pevt->p, pevt->inrange, pevt->tip, pevt->flag);
    }
...
static int fts_read_parse_touchdata(struct fts_ts_data *data)
{
    ...
    if (data->ic_info.is_incell) {
        if ((data->point_num == 0x0F) && (buf[2] == 0xFF) && (buf[3] == 0xFF) && (buf[4] == 0xFF) && (buf[5] == 0xFF) && (buf[6] == 0xFF)) {
...
static irqreturn_t fts_irq_handler(int irq, void *data)
{
    ...
    if (ret < 0) {
        ...
        FTS_DEBUG("fts_read 0x80=%x", value);
    }
...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    FTS_INFO("max touch number:%d, power gpio:%d, irq gpio:%d, reset gpio:%d", pdata->max_touch_number, pdata->power_gpio, pdata->irq_gpio, pdata->reset_gpio);
...
static int fts_ts_suspend(struct device *dev)
{
    ...
    FTS_INFO("fts_ts_suspend fb_power_off():%d\n", fb_power_off());
...
static int fts_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    // if (ratta_touch_exist(RATTA_TOUCH_IC_FT5XX)) {
...
#if 0
    //20210723: proc_symlink 无法建立 到 /sys 目录下的节点，只能建立 /proc 下面的连接。
    // 此处禁止是屏蔽 __xlate_proc_name 里面的 WARNING: at fs/proc/generic.c:161
    memset(buf, 0, sizeof(buf));
    snprintf(buf, sizeof(buf) - 1, "/sys/bus/i2c/devices/%s/fts_fw_version", kobject_name(&client->dev.kobj));
    ...
    //ratta_touch_set_ic(RATTA_TOUCH_IC_FT5XX);
