...
#if (FTS_CHIP_TYPE == _FT5936)
    &upgrade_func_ft5682,
#elif (FTS_CHIP_TYPE == _FT5426_003)
...
static int fts_fwupg_get_boot_state(struct fts_upgrade *upg, enum FW_STATUS *fw_sts)
{
    ...
    if (fts_data->ic_info.is_incell || (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)) {
...
static int fts_pram_ecc_cal(struct fts_upgrade *upg, u32 saddr, u32 len)
{
    ...
    if ((ECC_CHECK_MODE_CRC16 == upg->func->pram_ecc_check_mode) || (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)) {
...
int fts_fwupg_ecc_cal(u32 saddr, u32 len)
{
    ...
    ecc_len = 1;
    if ((ECC_CHECK_MODE_CRC16 == upg->func->fw_ecc_check_mode) || (upg->func->upgspec_version >= UPGRADE_SPEC_V_1_0)) {
        ecc_len = 2;
    }
...
int fts_fwupg_init(struct fts_ts_data *ts_data)
{
    ...
    queue_delayed_work(ts_data->ts_workqueue, &ts_data->fwupg_work, msecs_to_jiffies(1000));
