...
{
	int i;
	ts_debug("goodix_ts_report_pen status:%d x:%d y:%d\n",pen_data->coords.status,pen_data->coords.x,pen_data->coords.y);

    ts_debug("goodix_ts_report_pen: status:%d[N:0,U:1,D:2] x:%d y:%d,p:%d\n", pen_data->coords.status,
...
static irqreturn_t goodix_ts_threadirq_func(int irq, void *data)
{
	...
	ts_debug("goodix_ts_threadirq_func pen:%d type:%d\n",ts_dev->board_data.pen_enable,ts_event->event_type);
	...
	irq_flag[0] = 0;
	irq_flag[1] = 0;
	ts_dev->hw_ops->write(ts_dev, ts_dev->reg.coor, irq_flag, 2);
