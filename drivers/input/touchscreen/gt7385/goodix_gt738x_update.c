			   ...
			   struct firmware_data *fw_data)
{
	...
	/* Check ISP Run */
	ret = goodix_wait_sta(ts_dev, HW_REG_DOWNL_STATE, 0xff, 1000);
	if (ret < 0) {
		ts_err("%s: phoenix fw update isp don't run\n",__func__);
		ret = goodix_reg_read(ts_dev, 0x4195, d_data, 5);
		if (ret < 0) {
			ts_err("%s:read 0xffe0 failed\n",__func__);
			goto exit;
		}
		ts_info("read 0x4195 2 bytes:0x%02x,0x%02x,0x%02x,0x%02x,0x%02x",d_data[0], 
				d_data[1], d_data[2], d_data[3], d_data[4]);
		goto exit;
	}
	ret = 0;
