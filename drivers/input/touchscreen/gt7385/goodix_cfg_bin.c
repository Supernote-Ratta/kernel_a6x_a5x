...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
	...
	if (checksum != cfg_bin->head.checksum) {
		ts_err("cfg_bin checksum check filed 0x%02x != 0x%02x,pgk_num=%d",
		       cfg_bin->head.checksum, checksum, cfg_bin->head.pkg_num);
		//r = -EINVAL;  // 20210316,hsl fix.
		//goto exit;
	}
...
exit:
	if(cfg_bin->cfg_pkgs) kfree(cfg_bin->cfg_pkgs);
	if(cfg_bin->cfg_pkgs) kfree(cfg_bin->bin_data);
	kfree(cfg_bin);
	if (r)
			   ...
			   struct goodix_cfg_bin *cfg_bin)
{
	...
	for (i = 0; i < cfg_bin->head.pkg_num; i++) {
		...
		ts_info("ic_type:%d,pkg_num=%d", ts_dev->ic_type, cfg_bin->head.pkg_num);
