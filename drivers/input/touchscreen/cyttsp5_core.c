...
#include <linux/htfy_dbg.h>  // 20191031,hsl add.20191107--for function-call.
#include <linux/timekeeping.h> 
#ifdef CONFIG_TOUCHSCREEN_RATTA
#include <linux/input/ratta_touch.h>
#endif
//#define NO_ELINK
// 20190324：一般是由于没有接TP造成的TIMEOUT，此处不需要多次，以加快启动时间。
#define CY_CORE_STARTUP_RETRY_COUNT		3 // 3


// 20210630,hsl add for T2-COVER.
#define CY_CORE_SUPPORT_LO_FOR_COVER	0
...
#if CY_CORE_SUPPORT_LO_FOR_COVER

static u64	last_valid_touch_ns;

static void print_touch_input(struct cyttsp5_core_data *cd, int size)
{
	bool 	is_lo = (cd->input_buf[5] & (1<<5))? 1: 0;
	int 	rec_num = (cd->input_buf[5] & 0x1f);
	// int 	rec_len = 10;  // every record len 10 byte.
	u8		*rec0_start = &cd->input_buf[7];
	int 	x;
	int 	y;
	int 	tid;
	int 	leftoff;
	...
	printk("input_buf(len=%d): %*ph,rec_num=%d\n", size, size, cd->input_buf, rec_num);
	if(is_lo) {
		if(rec_num > 0) {
			x = rec0_start[2] | (rec0_start[3]<<8);
			y = rec0_start[4] | (rec0_start[5]<<8);
			tid = (rec0_start[0] & 0X1F);
			leftoff = (rec0_start[0] & 0X80)?1:0;
		} else {
			x = y = -1;
			tid = leftoff = 0;
		}
		off_ms = (ktime_get_ns() - last_valid_touch_ns)/1000000;
	/*
[  124.997725] cytt: Got LO,x=1,y=1,tid=0,leftoff=0,rec_num=0
								  len    RID  timestamp	 LO/NUM=0	RCN
[  124.997815] input_buf(len=7): 07 00   01    89 e0     20 		00

[  133.726619] cytt: Got LO,x=1,y=1,tid=0,leftoff=0,rec_num=3
[  133.726642] input_buf(len=37): 
	 len   RID  TS	  LO+NUM=3  RCN   
	25 00  01  f4 02 	23      80   

	REC0
	TY	TIP/TID	  X		  Y	   PRE   AIX   ORI
	00 	c2 		34 04 	03 06  2f 	12 12  00 
	00  80     52 04    75 05  31   12 12  00 
	00  81     12 04    bc 04  31   12 12  00	

[  133.753253] cytt: Got LO,x=1,y=1,tid=0,leftoff=0,rec_num=0
[  133.753275] input_buf(len=7): 07 00 01 10 03 20 00

	*/
		printk("cytt:Got_LO,x=%d,y=%d,tid=%d,leftoff=%d,rec_num=%d,off_ms=%d\n", x, y, 
			tid, leftoff, rec_num, off_ms);

		if(off_ms > 750) {
			schedule_delayed_work(&cd->cover_det_work, msecs_to_jiffies(250)); 
		}
	}else {
		last_valid_touch_ns = ktime_get_ns();
		cancel_delayed_work(&cd->cover_det_work);
	}
}
#endif 
...
#if 0
    if( cd->tch_disabled ) {
        //dev_err(cd->dev, "%s: Touch is Disabled!\n", __func__);
        if( cd->input_before_disabled ){
            call_atten_cb(cd, CY_ATTEN_STARTUP, 0);
            cd->input_before_disabled = false;
        }
        //call_atten_cb(cd, CY_ATTEN_STARTUP, 0);
        return rc;
    }
#endif
    // 20180701,LOG:
    // parse_touch_input: enter,id=1,tch=1,btn=3,size=17
    // parse_touch_input: enter,id=1,tch=1,btn=3,size=7   -- touch up.
    // input_buf (len=7): 07 00 01 87 ee 00 c0
    // input_buf (len=7): 07 00 01 f0 ee 00 80
    // input_buf (len=17): 11 00 01 b2 ee 01 80 00 60 a8 03 78 02 1b 41 07 f3
    // input_buf (len=27): 1b 00 01 e3 ee 02 40 00 60 44 03 6c 03 ff 07 07 00 00 61 f1 03 d4 02 1b 49 0f e0
    /*printk("%s: enter,id=%d,tch=%d,btn=%d\n", __func__, report_id,
        si->desc.tch_report_id, si->desc.btn_report_id);
    printk("input_buf (len=%d): %*ph\n", size, size, cd->input_buf);
    */

	if (report_id == si->desc.tch_report_id) {
	#if CY_CORE_SUPPORT_LO_FOR_COVER
		//printk("input_buf(len=%d):LO=%d,x=%d,y=%d\n", size, size, 
		//	cd->input_buf[5] & (1<<5) );
		print_touch_input(cd, size);
	#endif 
...
read:
	...
	if (rc) {
		dev_err(dev, "%s: Error getting report, r=%d,err_cnt=%d\n",
				__func__, rc, cd->i2c_err_cnt++);
		cd->i2c_err_cnt++;
		if(cd->i2c_err_cnt > 20) {
			cd->i2c_err_cnt = 20;
		}
		return rc;
	}
...
static irqreturn_t cyttsp5_irq(int irq, void *handle)
{
    ...
    // 20210605: 在没有触摸的情况下，也会定时1秒左右打印这个信息.
    // cyttsp5_irq: enter,state=FB_ON,tch_disabled=0
    // printk("%s: enter,state=%s,tch_disabled=%d\n", __func__, cd->fb_state == FB_ON?"FB_ON":"FB_OFF", cd->tch_disabled);
	...
	if( !cd->tch_disabled ) {
		...
		// 20180604-LOG: cyttsp5_irq: enter,state=FB_ON,rc=0 happend at 12.5HZ.
		// printk("%s: enter,state=%s,rc=%d\n", __func__, cd->fb_state == FB_ON?"FB_ON":"FB_OFF", rc);
		if (!rc)
...
static void cyttsp5_pm_resume_work_function(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =  container_of(work,
		struct cyttsp5_core_data, pm_resume_work.work);

	printk("%s:fb_state=%d(FB_ON=%d)\n", __func__, cd->fb_state, FB_ON);
	if (cd->irq_disabled) {
		enable_irq(cd->irq);
		cd->irq_disabled = 0;
	}
...
extern void rk_send_sleep_key(void);
static void cyttsp5_cover_work_function(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =  container_of(work,
		struct cyttsp5_core_data, cover_det_work.work);

	printk("%s:report-sleep-key,fb_state=%d\n", __func__, cd->fb_state);
...
static int cyttsp5_core_resume(struct device *dev)
{
	...
	// 20200919: 这个还是也会在亮屏/灭屏的时候调用。
    // printk("%s:enable irq by pm_resume_work,fb_state=%d(FB_ON=%d)\n", __func__, cd->fb_state, FB_ON);
	if( cd->fb_state == FB_ON ) {
		//cancel_delayed_work(&cd->pm_resume_work);
		#if 0
		/*
	...
	exit:
		...
		cyttsp5_core_wake(cd);
		#else
		schedule_delayed_work(&cd->pm_resume_work, 0); // msecs_to_jiffies(30)
		#endif
	} else {
		// 20200919: 这里面的延迟时间必须大于 i2c-hid.c::resume 函数里面的延迟时间，因为两者有依赖关系。
		// 20210413: 减少这个延迟，主要是休眠唤醒后能够比较快就可以触摸。同时需要修改 h2c-hid.c: i2c_hid_resume
		schedule_delayed_work(&cd->pm_resume_work, msecs_to_jiffies(20)); // 120
	}

	return 0;
}

static int cyttsp5_core_suspend_pm(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	printk("%s:fb_state=%d(FB_ON=%d),may wakeup=%d\n", __func__,
		cd->fb_state, FB_ON, device_may_wakeup(dev));
	if (cd->fb_state == FB_ON) {
		if (device_may_wakeup(dev)) {
			parade_debug(dev, DEBUG_LEVEL_2, "%s Device MAY wakeup\n",
				__func__);
...
static int cyttsp5_core_resume_pm(struct device *dev)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	printk("%s:fb_state=%d(FB_ON=%d),irq_wake=%d\n", __func__,
		cd->fb_state, FB_ON, cd->irq_wake);
	if (device_may_wakeup(dev)) {
		parade_debug(dev, DEBUG_LEVEL_2, "%s Device MAY wakeup\n",
			__func__);
		if (cd->irq_wake) {
...
static void cyttsp5_resume_work_function(struct work_struct *work)
{
	struct cyttsp5_core_data *cd =  container_of(work,
...
const struct dev_pm_ops cyttsp5_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(cyttsp5_core_suspend_pm, cyttsp5_core_resume_pm)
		...
		unsigned long event, void *data)
{
	...
	dbg_printk(IDBG_WACOM,"cyttsp5_fb_notify: event=%ld, blank=%d\n", event, blank);

	// 20191207：当 笔开始点击的时候，我们关闭TP消息。这里不能直接关闭电源，因为用户可能需要快速从笔
	// 操作转为 TP操作。
	if( blank == FB_BLANK_VSYNC_SUSPEND ){
	    //printk("%s:disable touch\n", __func__ );
	    if (cd->irq_enabled) {
			cd->irq_enabled = false;
			//disable_irq_nosync(cd->irq);
			disable_irq(cd->irq);
			...
			// 20200716-LOG:
			// cyttsp5_i2c_adapter 1-0024: cyttsp5_hid_exec_cmd_and_wait_: HID output cmd execution timed out
			// cyttsp5_i2c_adapter 1-0024: cyttsp5_hid_cmd_set_power_: Failed to set power to state:1
			//cyttsp5_put_device_into_sleep_(cd);
		}

		// 20200612: 我们需要达到的效果是用户开始用笔写的时候，无论是TP有多少个点按下，都处于无效状态。
		// 当笔抬起的时候，TP需要重新按下才有效(否则会出现手写自动翻页等意外行为)。如果落笔的时候，此处模拟
		// 了TP弹起操作，则当抬笔的时候，即使手腕没有操作，也会自动产生一个FLINGER 的 DOWN/UP 事件，这个是我们
		// 不希望的。所以此处不能调用 call_atten_cb 来模拟谈起事件。这样会不会存在手写的过程中TP弹起了，但是系统并
		// 不知道，当再次按下TP的时候，上层的 TP TOUCH-DOWN 状态发生错误导致 TP无效呢？我们修改了 inputDispather ,
		// 当 落笔的时候，同时清除 TP 的down状态，这样既可以防止后续TP事件起作用，也可以避免 TP 弹起后再触摸无效。
		// 20200615：实际上在笔落下的时候我们需要丢弃左右的TP信息，且在TP没有抬起之前，后续信息也是无效的。这样此处
		// 反而不能模拟 TP UP 状态。否则会导致上层应用误触发。前面碰到的TP无效的问题，应该是 dispatch 流程冲突的问题.
		//mutex_lock(&cd->irq_lock);
		cd->tch_disabled = 1;
		//call_atten_cb(cd, CY_ATTEN_STARTUP, 0); // 20200609: use to release all mt-touches.
		//mutex_unlock(&cd->irq_lock);
	} else if( FB_BLANK_NORMAL == blank ) {
    	cd->tch_disabled = 0;
    	//printk("%s:enable touch\n", __func__ );
    	if (!cd->irq_enabled) {
    		// 20200819: 测试反馈，有概率性 TP 无响应，getevent 没有事件上报。是否是 tp 异常了？
    		// 我们在 tp enable 的时候，读取一下数据看看。我们在 irq - enable 之前处理，防止重入。
    		int rc = cyttsp5_read_input(cd);
			if (!rc) {
				cyttsp5_parse_input(cd);
			} else {
				// read error.do reset.
				printk("%s:cyttsp5_read_inputx error, rc=%d\n", __func__, rc);
				cyttsp5_core_sleep(cd);
				msleep(150);
				cyttsp5_core_wake(cd);
			}
			...
			//cyttsp5_core_wake_device_(cd); // 20200716:don't send any cmd.
			cyttsp5_start_wd_timer(cd);
		}
		...
		if (cd->fb_state != FB_ON) {
		    ...
		    //dev_info(cd->dev, "%s: cyttsp5_core_resume!\n", __func__);
		    //cyttsp5_core_wake(cd);
		    //cyttsp5_core_resume(cd->dev);
			//call_atten_cb(cd, CY_ATTEN_RESUME, 0);
			cd->fb_state = FB_ON;
			...
			// 20191226,use work for fast-wakeup.
		...
		if (cd->fb_state != FB_OFF) {
		    //dev_info(cd->dev, "%s: CY_ATTEN_SUSPEND-cyttsp5_core_suspend!\n", __func__);
			call_atten_cb(cd, CY_ATTEN_SUSPEND, 0);
			//cyttsp5_core_sleep(cd);
			cyttsp5_core_suspend(cd->dev);
			cd->fb_state = FB_OFF;

			// 20191224,set for deep sleep.
		...
		u16 irq, size_t xfer_buf_size)
{
...
#ifdef CONFIG_TOUCHSCREEN_RATTA
	if (ratta_touch_exist(RATTA_TOUCH_IC_CYTTSP5)) {
		dev_info(dev, "Already have touchscreen.\n");
	...
	INIT_WORK(&cd->resume_work, cyttsp5_resume_work_function);

	// 20200919,hsl add.
	INIT_DELAYED_WORK(&cd->pm_resume_work, cyttsp5_pm_resume_work_function);

	INIT_DELAYED_WORK(&cd->cover_det_work, cyttsp5_cover_work_function);
...
#endif
    // 20180702,hsl change to disable: 1--> 0
	...
	is_cyttsp5_probe_success = true;
#ifdef CONFIG_TOUCHSCREEN_RATTA
	ratta_touch_set_ic(RATTA_TOUCH_IC_CYTTSP5);
#endif
	return 0;
