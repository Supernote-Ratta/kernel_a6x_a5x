...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
	...
	if(ts->revert_y_flag)
		input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, ts->abs_x_max-y);
	else
    ...
    GTP_DEBUG("ID:%d, xmax:%d ymax:%d X:%d, Y:%d, W:%d", id, ts->abs_x_max,ts->abs_y_max,x, y, w);
}
...
//#endif
	...
	if(ts->revert_y_flag){
        input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y, ts->abs_x_max-y);
    }
    else{
...
//#endif
	...
	// 20200905-hsl: rm the stylus property of the input.
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ts->revert_x_flag = 0;
    if (!of_property_read_u32(client->dev.of_node, "revert_x_flag", &val))
        ts->revert_x_flag = !!val;
    ts->revert_y_flag = 0;
    if (!of_property_read_u32(client->dev.of_node, "revert_y_flag", &val))
        ts->revert_y_flag = !!val;
    ts->exchange_x_y_flag = 0;
    if (!of_property_read_u32(client->dev.of_node, "exchange_x_y_flag", &val))
        ts->exchange_x_y_flag = !!val;
...
static int gtp_fb_notifier_callback(struct notifier_block *noti, unsigned long event, void *data)
{
	...
	if (ev_data && ev_data->data && event == FB_EVENT_BLANK && ts) {
		...
		if (*blank == FB_BLANK_UNBLANK) {
			...
			GTP_DEBUG("Suspend by fb notifier.");//深度休眠 tanlq
			ts->fb_off = 1;
...
if(ev_data && ev_data->data && event == FB_EARLY_EVENT_BLANK && ts) {
	...
	case FB_BLANK_VSYNC_SUSPEND:
		//if(ts->is_enable){
			...
			GTP_INFO("%s:disable touch\n", __func__ );
		//}
		break;
	case FB_BLANK_NORMAL:
		//if(!ts->is_enable){
		//	ts->is_enable = true;
			GTP_INFO("%s:enable touch\n", __func__ );
			gtp_irq_enable(ts);
		//}
		break;
	default:
...
} else if( ev_data && ev_data->data && event == FB_EVENT_BLANK && ts) {
	switch (*blank) {
...
static int gtp_pm_suspend(struct device *dev)
{
	...
	if (ts && !ts->fb_off) {
		GTP_DEBUG("Suspend by i2c pm."); //亮屏休眠 tanlq
		//goodix_ts_suspend(ts);
		enable_irq_wake(client->irq);
	}
...
//must late than pen,or i2c can't run correctly.
