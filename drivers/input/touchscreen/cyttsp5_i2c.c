...
static int cyttsp5_power_switch(struct i2c_client *client, int on)
{
	static struct regulator *vdd_ana;
	int ret;

	if (!vdd_ana) {
		vdd_ana = devm_regulator_get(&client->dev, "vcc_tp");
			//regulator_get(&client->dev, "vcc_tp");
		if (IS_ERR(vdd_ana)) {
			printk("cyttsp5 regulator get of vdd_ana failed \n");
			ret = PTR_ERR(vdd_ana);
			vdd_ana = NULL;
			return ret;
		}
...
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_DEVICETREE_SUPPORT
    rc = cyttsp5_power_switch(client, 1);
	if (rc) {
		printk("cyttsp5 power on failed.\n");
		return -EINVAL;
	}
...
static int __init cyttsp5_i2c_init(void)
{
	...
	//printk("cyttsp5_i2c_init");
	return rc;
}
//module_init(cyttsp5_i2c_init); // 20191230,wait for regulator.
late_initcall_sync(cyttsp5_i2c_init); //20200330,need to make sure wacom is init first then TP.
