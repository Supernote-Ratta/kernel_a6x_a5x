...
static int async_read(struct goodix_tools_dev *dev, void __user *arg)
{
    ...
    reg_addr = i2c_msg_head[0] + (i2c_msg_head[1] << 8) + (i2c_msg_head[2] << 16) + (i2c_msg_head[3] << 24);
    length = i2c_msg_head[4] + (i2c_msg_head[5] << 8) + (i2c_msg_head[6] << 16) + (i2c_msg_head[7] << 24);
...
static int sync_read(struct goodix_tools_dev *dev, void __user *arg)
{
    ...
    tools_data.reg_addr = i2c_msg_head[0] + (i2c_msg_head[1] << 8) + (i2c_msg_head[2] << 16) + (i2c_msg_head[3] << 24);
    tools_data.length = i2c_msg_head[4] + (i2c_msg_head[5] << 8) + (i2c_msg_head[6] << 16) + (i2c_msg_head[7] << 24);
    tools_data.filled = 0;
    ...
    if (tools_data.filled == 1) {
        if (copy_to_user((u8 *)arg + I2C_MSG_HEAD_LEN, tools_data.data, tools_data.length)) {
            ret = -EFAULT;
            ts_err("Copy_to_user failed");
        } else {
...
static long goodix_tools_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd & NEGLECT_SIZE_MASK) {
        ...
        case GTP_SEND_COMMAND:
            ret = copy_from_user(&temp_cmd, (void __user *)arg, sizeof(struct goodix_ts_cmd));
            if (ret) {
                ret = -EINVAL;
                goto err_out;
            }
        ...
        case GTP_DRV_VERSION:
            ret = copy_to_user((u8 *)arg, GOODIX_DRIVER_VERSION, sizeof(GOODIX_DRIVER_VERSION));
            if (ret) {
...
static int goodix_tools_module_irq(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    ...
    if (!list_empty(&dev->head)) {
        ...
        list_for_each_entry_safe(tools_data, next, &dev->head, list) {
            if (!hw_ops->read_trans(ts_dev, tools_data->reg_addr, tools_data->data, tools_data->length)) {
                tools_data->filled = 1;
            }
        }
        wake_up(&dev->wq);
