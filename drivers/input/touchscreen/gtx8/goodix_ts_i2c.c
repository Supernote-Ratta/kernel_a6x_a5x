 ...
 */
static int goodix_parse_dt_resolution(struct device_node *node, struct goodix_ts_board_data *board_data)
{
    int r, err;
...
static int goodix_parse_dt(struct device_node *node, struct goodix_ts_board_data *board_data)
{
    ...
    if (!r) {
        ...
        if (strlen(name_tmp) < sizeof(board_data->avdd_name)) {
            ...
            ts_info("invalied avdd name length: %ld > %ld", strlen(name_tmp), sizeof(board_data->avdd_name));
        }
    }
    ...
    if (prop && prop->length) {
        ...
        r = of_property_read_u32_array(node, "goodix,panel-key-map", &board_data->panel_key_map[0], board_data->panel_max_key);
        if (r) {
    ...
    ts_info("***key:%d, %d, %d, %d", board_data->panel_key_map[0], board_data->panel_key_map[1], board_data->panel_key_map[2], board_data->panel_key_map[3]);

    ts_debug("[DT]x:%d, y:%d, w:%d, p:%d", board_data->panel_max_x, board_data->panel_max_y, board_data->panel_max_w, board_data->panel_max_p);
    return 0;
}
#endif
 ...
 */
int goodix_i2c_read_trans(struct goodix_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len)
{
    struct i2c_client *client = to_i2c_client(dev->dev);
    ...
    if (likely(len < sizeof(get_buf))) {
        ...
        msgs[1].buf = kzalloc(I2C_MAX_TRANSFER_SIZE < len ? I2C_MAX_TRANSFER_SIZE : len, GFP_KERNEL);
        if (msgs[1].buf == NULL) {
            return -ENOMEM;
        }
    }
 ...
 */
int goodix_i2c_write_trans(struct goodix_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len)
{
    struct i2c_client *client = to_i2c_client(dev->dev);
    ...
    if (likely(len + TS_ADDR_LENGTH < sizeof(put_buf))) {
        ...
        msg.buf = kmalloc(I2C_MAX_TRANSFER_SIZE < len + TS_ADDR_LENGTH ? I2C_MAX_TRANSFER_SIZE : len + TS_ADDR_LENGTH, GFP_KERNEL);
        if (msg.buf == NULL) {
            return -ENOMEM;
        }
    }
 ...
 * return: 0 - write ok; < 0 - i2c transter error.
 */
int goodix_i2c_write(struct goodix_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len)
{
    int r = -EINVAL;
 ...
 * return: 0 - read ok, < 0 - i2c transter error
 */
int goodix_i2c_read(struct goodix_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len)
{
    int r = -EINVAL;
 ...
 * return: 0 - write ok; < 0 - i2c transter error.
 */
int goodix_i2c_write_trans_once(struct goodix_ts_device *dev, unsigned int reg, unsigned char *data, unsigned int len)
{
    struct i2c_client *client = to_i2c_client(dev->dev);
    unsigned int pos = 0, transfer_length = 0;
    unsigned int address = reg;
...
static int goodix_read_version(struct goodix_ts_device *dev, struct goodix_ts_version *version)
{
    ...
    if (!dev->reg.pid || !dev->reg.sensor_id || !dev->reg.vid) {
        ts_err("reg is NULL, pid:0x%04x, vid:0x%04x, sensor_id:0x%04x", dev->reg.pid, dev->reg.vid, dev->reg.sensor_id);
        return -EINVAL;
    }
    if (!pid_read_len || pid_read_len > GOODIX_PID_MAX_LEN || !vid_read_len || vid_read_len > GOODIX_VID_MAX_LEN) {
        ts_err("invalied pid vid length, pid_len:%d, vid_len:%d", pid_read_len, vid_read_len);
        return -EINVAL;
    }
    ...
    if (dev->reg.version_base && dev->reg.version_len < 256) {
        r = goodix_i2c_read(dev, dev->reg.version_base, temp_buf, dev->reg.version_len);
        if (r < 0) {
            ts_err("Read version base failed, reg:0x%02x, len:%d", dev->reg.version_base, dev->reg.version_len);
            if (version) {
                version->valid = false;
            }
            goto exit;
        ...
        if (checksum) {
            ts_err("checksum error:0x%02x, base:0x%02x, len:%d", checksum, dev->reg.version_base, dev->reg.version_len);
            ts_err("%*ph", (int)(dev->reg.version_len / 2), temp_buf);
            ts_err("%*ph", (int)(dev->reg.version_len - dev->reg.version_len / 2), &temp_buf[dev->reg.version_len / 2]);
    ...
    if (sensor_id_mask != 0) {
        ...
        ts_info("sensor_id_mask:0x%02x, sensor_id:0x%02x", sensor_id_mask, version->sensor_id);
    } else {
    ...
    ts_info("PID:%s,SensorID:%d, VID:%*ph", version->pid, version->sensor_id, (int)sizeof(version->vid), version->vid);
exit:
...
static int goodix_wait_cfg_cmd_ready(struct goodix_ts_device *dev, u8 right_cmd, u8 send_cmd)
{
    int try_times = 0;
    u8 cmd_flag = 0;
    u8 cmd_buf[3] = {0};
    ...
    for (try_times = 0; try_times < TS_WAIT_CFG_READY_RETRY_TIMES; try_times++) {
        ...
        if (cmd_flag == right_cmd) {
            ...
            ts_err("failed cmd_reg:0x%X, 0x%X, 0x%X", cmd_buf[0], cmd_buf[1], cmd_buf[2]);
...
static int goodix_send_small_config(struct goodix_ts_device *dev, struct goodix_ts_config *config)
{
    int r = 0;
    int try_times = 0;
    u8 buf = 0;
    ...
    if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
        ts_err("failed send small cfg, reg:0x%04x is not 0xff", command_reg);
        r = -EINVAL;
        goto exit;
    }
...
static int goodix_send_large_config(struct goodix_ts_device *dev, struct goodix_ts_config *config)
{
    int r = 0;
    int try_times = 0;
    u8 buf = 0;
    ...
    if (try_times >= TS_WAIT_CMD_FREE_RETRY_TIMES) {
        ts_err("failed send large cfg, reg:0x%04x is not 0xff", command_reg);
        r = -EINVAL;
        goto exit;
    }
    ...
    if (goodix_wait_cfg_cmd_ready(dev, COMMAND_SEND_CFG_PREPARE_OK, COMMAND_START_SEND_CFG)) {
        ts_err("failed send large cfg, reg:0x%04x is not 0x82", command_reg);
        r = -EINVAL;
        goto exit;
    }
...
static int goodix_check_cfg_valid(struct goodix_ts_device *dev, u8 *cfg, u32 length)
{
    ...
    if (dev->ic_type == IC_TYPE_NANJING) {
        ...
        if (checksum != 0) {
            ts_err("cfg head checksum ERROR, checksum:0x%02x", checksum);
            ret = -EINVAL;
            goto exit;
        }
        bag_num = cfg[1];
    ...
    for (j = 0; j < bag_num; j++) {
        ...
        checksum = 0;
        if (bag_end > length) {
            ts_err("ERROR, overflow!!bag:%d, bag_start:%d, bag_end:%d, cfg length:%d", j, bag_start, bag_end, length);
            ret = -EINVAL;
            goto exit;
        }
...
static int goodix_send_config(struct goodix_ts_device *dev, struct goodix_ts_config *config)
{
    int r = 0;
    ...
    if (r != 0) {
        ts_err("send_cfg FAILED, ic_type:%d, cfg_len:%d", dev->ic_type, config->length);
    }
        ...
        if (ts_cmd.initialized) {
            ...
            for (j = 0; j < 3; j++) {
                if (goodix_i2c_read(ts_dev, 0x8040, buffer, 1) != 0) {
                    ...
                    if (buffer[0] != 0xFF) {
                        ts_info("try_times:%d:%d, read 0x8040:0x%02x", try_times, j, buffer[0]);
                        usleep_range(10000, 11000);
                        continue;
                    } else {
                        goto exit;
...
static int _goodix_do_read_config(struct goodix_ts_device *dev, u32 base_addr, u8 *buf)
{
    ...
    if (checksum) {
        ts_err("Config head checksum err:0x%x,data:%*ph", checksum, TS_CFG_HEAD_LEN, buf);
        ret = -EINVAL;
        goto err_out;
    }
    ...
    for (i = 0; i < sub_bags; i++) {
        ...
        ret = goodix_i2c_read(dev, base_addr + offset + 2, buf + offset + 2, subbag_len + 1);
        if (ret) {
            goto err_out;
        }
        ...
        offset += subbag_len + 3;
        ts_debug("sub bag %d, data:%*ph", buf[offset], buf[offset + 1] + 3, buf + offset);
    }
    ret = offset;
...
static int goodix_read_config(struct goodix_ts_device *dev, u8 *config_data, u32 config_len)
{
    struct goodix_ts_cmd ts_cmd;
    u8 cmd_flag;
...
static void goodix_swap_coords(struct goodix_ts_device *dev, unsigned int *coor_x, unsigned int *coor_y)
{
    unsigned int temp;
...
static void goodix_parse_finger(struct goodix_ts_device *dev, struct goodix_touch_data *touch_data, unsigned char *buf, int touch_num)
{
    ...
    for (i = 0; i < touch_num; i++) {
        ...
        x = coor_data[i * BYTES_PER_COORD + 1] | coor_data[i * BYTES_PER_COORD + 2] << 8;
        y = coor_data[i * BYTES_PER_COORD + 3] | coor_data[i * BYTES_PER_COORD + 4] << 8;
...
static void goodix_parse_pen(struct goodix_ts_device *dev, struct goodix_pen_data *pen_data, unsigned char *buf, int touch_num)
{
    unsigned int id = 0;
    static u8 pre_key_map;
    u8 cur_key_map = 0;
    ...
    for (i = 0; i < touch_num; i++) {
        ...
        pen_data->coords.x = coor_data[i * BYTES_PER_COORD + 1] | coor_data[i * BYTES_PER_COORD + 2] << 8;
        pen_data->coords.y = coor_data[i * BYTES_PER_COORD + 3] | coor_data[i * BYTES_PER_COORD + 4] << 8;
        pen_data->coords.p = coor_data[i * BYTES_PER_COORD + 5] | coor_data[i * BYTES_PER_COORD + 6] << 8;
        goodix_swap_coords(dev, &pen_data->coords.x, &pen_data->coords.y);
...
static int goodix_touch_handler(struct goodix_ts_device *dev, struct goodix_ts_event *ts_event, u8 *pre_buf, u32 pre_buf_len)
{
    ...
    if (unlikely(touch_num > 1)) {
        r = goodix_i2c_read_trans(dev, dev->reg.coor + 4 + BYTES_PER_COORD, &buffer[4 + BYTES_PER_COORD], (touch_num - 1) * BYTES_PER_COORD);
        if (unlikely(r < 0)) {
            goto exit_clean_sta;
        }
    }
    ...
    if (touch_num >= 1 && buffer[(touch_num - 1) * BYTES_PER_COORD + 2] >= 0x80) {
        if (pre_finger_num) {
...
static int goodix_event_handler(struct goodix_ts_device *dev, struct goodix_ts_event *ts_event)
{
    unsigned char pre_buf[4 + BYTES_PER_COORD];
    unsigned char event_sta;
    int r;

    r = goodix_i2c_read_trans(dev, dev->reg.coor, pre_buf, 4 + BYTES_PER_COORD);
    if (unlikely(r < 0)) {
        return r;
    }
...
static int goodix_i2c_probe(struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    if (IS_ENABLED(CONFIG_OF) && client->dev.of_node) {
        ...
        r = goodix_parse_dt(client->dev.of_node, &ts_device->board_data);
        if (r < 0) {
