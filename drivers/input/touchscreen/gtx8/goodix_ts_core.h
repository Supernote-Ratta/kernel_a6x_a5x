...
/* macros definition */
#define GOODIX_CORE_DRIVER_NAME            "goodix_ts_core"
...
struct goodix_ts_hw_ops {
    ...
    int (*read_config)(struct goodix_ts_device *dev, u8 *config_data, u32 config_len);
 ...
 */
static inline struct goodix_ts_board_data *board_data(struct goodix_ts_core *core)
{
    if (!core || !core->ts_dev) {
        return NULL;
    }
 ...
 */
static inline const struct goodix_ts_hw_ops *ts_hw_ops(struct goodix_ts_core *core)
{
    if (!core || !core->ts_dev) {
        return NULL;
    }
