...
static int goodix_parse_firmware(struct firmware_data *fw_data)
{
    ...
    if (checksum != fw_info->checksum) {
        ts_err("Bad firmware, cheksum error %x(file) != %x(cal)", fw_info->checksum, checksum);
        r = -EINVAL;
        goto err_size;
    }

    if (fw_info->subsys_num > FW_SUBSYS_MAX_NUM) {
        ts_err("Bad firmware, invalid subsys num: %d", fw_info->subsys_num);
        r = -EINVAL;
        goto err_size;
    }
    ...
    ts_info("Fimware VID:%02X%02X%02X%02x", fw_info->fw_vid[0], fw_info->fw_vid[1], fw_info->fw_vid[2], fw_info->fw_vid[3]);
 ...
 */
static int goodix_check_update(struct goodix_ts_device *dev, const struct firmware_info *fw_info)
{
 ...
 *         < 0 failed
 */
static int goodix_reg_write_confirm(struct goodix_ts_device *dev, unsigned int addr, unsigned char *data, unsigned int len)
{
    u8 *cfm, cfm_buf[32];
    int r, i;
    ...
    for (i = 0; i < GOODIX_BUS_RETRY_TIMES; i++) {
        ...
        if (memcmp(data, cfm, len)) {
            ts_info("data[0]:0x%02x, data[1]:0x%02x, read cfm[0]:0x%02x, cfm[1]:0x%02x", data[0], data[1], cfm[0], cfm[1]);
            dev->hw_ops->read_trans(dev, 0x6022, cfm, 2);
            ts_info("read 0x6022 data[0]:0x%02x, data[1]:0x%02x", cfm[0], cfm[1]);
            r = -EMEMCMP;
            continue;
        } else {
            r = 0;
...
static inline int goodix_reg_write(struct goodix_ts_device *dev, unsigned int addr, unsigned char *data, unsigned int len)
{
    return dev->hw_ops->write_trans(dev, addr, data, len);
}

static inline int goodix_reg_read(struct goodix_ts_device *dev, unsigned int addr, unsigned char *data, unsigned int len)
{
 ...
 * return 0 ok, <0 error
 */
static int goodix_load_isp(struct goodix_ts_device *ts_dev, struct firmware_data *fw_data)
{
    ...
    r = goodix_reg_write_confirm(ts_dev, HW_REG_ISP_ADDR, (u8 *)fw_isp->data, fw_isp->size);
    if (r < 0) {
    ...
    ts_err("ISP not work,0x%x=0x%x, 0x%x=0x%x", HW_REG_ISP_RUN_FLAG, reg_val[0], HW_REG_ISP_RUN_FLAG + 1, reg_val[1]);
    return -EFAULT;
}
...
static int goodix_update_prepare(struct fw_update_ctrl *fwu_ctrl)
{
    ...
    ts_debug("Success enabled DSP&MCU power,set 0x%x-->0x00", HW_REG_DSP_MCU_POWER);
 ...
 * @data: packet data
 */
static int goodix_format_fw_packet(u8 *pkt, u32 flash_addr, u16 len, const u8 *data)
{
    u16 checksum;
 ...
 * return：0 ok, <0 error
 */
static int goodix_send_fw_packet(struct goodix_ts_device *dev, u8 type, u8 *pkt, u32 len)
{
    u8 reg_val[4];
    int r, i;
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_subsystem(struct goodix_ts_device *dev, struct fw_subsys_info *subsys)
{
    ...
    while (total_size > 0) {
        ...
        ts_info("Flash firmware to %08x,size:%u bytes", subsys_base_addr + offset, data_size);
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_flash_firmware(struct goodix_ts_device *dev, struct firmware_data *fw_data)
{
    ...
    for (i = 1; i < fw_num && retry;) {
        ...
        if (r == 0) {
            ...
            retry--;
            ts_err("--- End flash subsystem%d: Fail, errno:%d, retry:%d ---", i, r, GOODIX_BUS_RETRY_TIMES - retry);
 ...
 * return: 0 ok, < 0 error
 */
static int goodix_update_finish(struct goodix_ts_device *ts_dev, struct fw_update_ctrl *fwu_ctrl)
{
    u8 reg_val[8] = {0};
    int r = 0;
...
static int goodix_flash_config(struct goodix_ts_device *ts_dev)
{
    ...
    /* format one firmware packet */
    ret = goodix_format_fw_packet(fw_packet, FLASH_ADDR_CONFIG_DATA, ISP_MAX_BUFFERSIZE, temp_data);
    if (ret < 0) {
        ts_err("Invalid packet params");
        goto exit;
    }
    ...
    ret = goodix_send_fw_packet(ts_dev, FLASH_SUBSYS_TYPE_CONFIG, fw_packet, ISP_MAX_BUFFERSIZE + 6);
    if (ret) {
 ...
 * return: 0 - OK, < 0 - error
 */
static int goodix_request_firmware(struct firmware_data *fw_data, const char *name)
{
    struct fw_update_ctrl *fw_ctrl = container_of(fw_data, struct fw_update_ctrl, fw_data);
 ...
 */
static ssize_t goodix_sysfs_update_en_store(struct goodix_ext_module *module, const char *buf, size_t count)
{
    int ret = 0;
    int mode = 0;
...
static ssize_t goodix_sysfs_update_progress_show(struct goodix_ext_module *module, char *buf)
{
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
    return scnprintf(buf, PAGE_SIZE, "%d\n", fw_ctrl->progress);
}

static ssize_t goodix_sysfs_update_result_show(struct goodix_ext_module *module, char *buf)
{
    char *result = NULL;
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
...
static ssize_t goodix_sysfs_update_fwversion_show(struct goodix_ext_module *module, char *buf)
{
    struct goodix_ts_version fw_ver;
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
    int r = 0;
...
static ssize_t goodix_sysfs_fwsize_show(struct goodix_ext_module *module, char *buf)
{
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
    int r = -EINVAL;
...
static ssize_t goodix_sysfs_fwsize_store(struct goodix_ext_module *module, const char *buf, size_t count)
{
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
    struct firmware *fw;
    u8 **data;
...
static ssize_t goodix_sysfs_fwimage_store(struct file *file, struct kobject *kobj, struct bin_attribute *attr, char *buf, loff_t pos, size_t count)
{
    struct fw_update_ctrl *fw_ctrl;
    struct firmware_data *fw_data;
...
static ssize_t goodix_sysfs_force_update_store(struct goodix_ext_module *module, const char *buf, size_t count)
{
    return count;
}
...
static int goodix_fw_sysfs_init(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    struct fw_update_ctrl *fw_ctrl = module->priv_data;
    ...
    ret = kobject_init_and_add(&module->kobj, ktype, &core_data->pdev->dev.kobj, "fwupdate");
    if (ret) {
...
static int goodix_fw_update_init(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    int ret = 0;
    ...
    if (ts_bdata && ts_bdata->fw_name) {
        strlcpy(goodix_fw_update_ctrl.fw_name, ts_bdata->fw_name, sizeof(goodix_fw_update_ctrl.fw_name));
    } else {
        strlcpy(goodix_fw_update_ctrl.fw_name, TS_DEFAULT_FIRMWARE, sizeof(goodix_fw_update_ctrl.fw_name));
    }
...
static int goodix_fw_update_exit(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    return 0;
}

static int goodix_fw_before_suspend(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
...
static int goodix_fw_before_resume(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    struct fw_update_ctrl *fwu_ctrl = module->priv_data;
...
static int goodix_fw_after_resume(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    return 0;
}

static int goodix_fw_irq_event(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    struct fw_update_ctrl *fwu_ctrl = module->priv_data;
...
static int goodix_fw_before_reset(struct goodix_ts_core *core_data, struct goodix_ext_module *module)
{
    struct fw_update_ctrl *fwu_ctrl = module->priv_data;
