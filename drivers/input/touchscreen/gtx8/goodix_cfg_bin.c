...
static int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin);
...
static int goodix_parse_cfg_bin(struct goodix_cfg_bin *cfg_bin)
{
    ...
    if (cfg_bin->bin_data_len != cfg_bin->head.bin_len) {
        ts_err("cfg_bin len check failed,%d != %d", cfg_bin->head.bin_len, cfg_bin->bin_data_len);
        r = -EINVAL;
        goto exit;
    }
    ...
    if (checksum != cfg_bin->head.checksum) {
        ts_err("cfg_bin checksum check filed 0x%02x != 0x%02x", cfg_bin->head.checksum, checksum);
        r = -EINVAL;
        goto exit;
    }
    ...
    for (i = 0; i < cfg_bin->head.pkg_num; i++) {
        ...
        if (i == cfg_bin->head.pkg_num - 1) {
            offset1 = cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN] + (cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 1] << 8);
            cfg_bin->cfg_pkgs[i].pkg_len = cfg_bin->bin_data_len - offset1;
        } else {
            offset1 = cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN] + (cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 1] << 8);
            offset2 = cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 2] + (cfg_bin->bin_data[TS_CFG_BIN_HEAD_LEN + i * TS_CFG_OFFSET_LEN + 3] << 8);
            if (offset2 <= offset1) {
        ...
        /*get cfg pkg head*/
        memcpy(&cfg_bin->cfg_pkgs[i].cnst_info, &cfg_bin->bin_data[offset1], TS_PKG_CONST_INFO_LEN);
        memcpy(&cfg_bin->cfg_pkgs[i].reg_info, &cfg_bin->bin_data[offset1 + TS_PKG_CONST_INFO_LEN], TS_PKG_REG_INFO_LEN);
    ...
    for (i = 0; i < cfg_bin->head.pkg_num; i++) {
        ...
        ts_debug("version base reg:%02x, len:%d", cfg_bin->cfg_pkgs[i].reg_info.version_base.addr, cfg_bin->cfg_pkgs[i].reg_info.version_base.reserved1);
        ts_debug("pid reg:%02x:%d", cfg_bin->cfg_pkgs[i].reg_info.pid.addr, cfg_bin->cfg_pkgs[i].reg_info.pid.reserved1);
        ts_debug("vid reg:%02x:%d", cfg_bin->cfg_pkgs[i].reg_info.vid.addr, cfg_bin->cfg_pkgs[i].reg_info.vid.reserved1);
        ts_debug("sensor_id reg:%02x,mask:%x", cfg_bin->cfg_pkgs[i].reg_info.sensor_id.addr, cfg_bin->cfg_pkgs[i].reg_info.sensor_id.addr);
...
static int goodix_extract_cfg_pkg(struct goodix_ts_device *ts_dev, struct goodix_cfg_package *cfg_pkg)
{
    ...
    ts_cfg->length = cfg_pkg->pkg_len - TS_PKG_CONST_INFO_LEN - TS_PKG_REG_INFO_LEN;
    if (ts_cfg->length > sizeof(ts_cfg->data)) {
        ts_err("illegal cfg length %d", ts_cfg->length);
        return -EINVAL;
    }
    if (ts_cfg->length) {
        ts_info("get config type %d, len %d", cfg_pkg->cnst_info.cfg_type, ts_cfg->length);
...
static int goodix_get_reg_and_cfg(struct goodix_ts_device *ts_dev, struct goodix_cfg_bin *cfg_bin)
{
    ...
    for (i = 0; i < cfg_bin->head.pkg_num; i++) {
        /*get ic type*/
        if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "nanjing", strlen("nanjing"))) {
            ts_dev->ic_type = IC_TYPE_NANJING;
        } else if (!strncmp(cfg_bin->cfg_pkgs[i].cnst_info.ic_type, "normandy", strlen("normandy"))) {
            ts_dev->ic_type = IC_TYPE_NORMANDY;
        } else {
            ts_err("unknow ic type of cfg_bin:%s", cfg_bin->cfg_pkgs[i].cnst_info.ic_type);
            continue;
        }
        ...
        if (temp_sensor_id != cfg_bin->cfg_pkgs[i].cnst_info.sensor_id) {
            ...
            ts_err("sensor_id from i2c:%d, sensor_id of cfg bin:%d", temp_sensor_id, cfg_bin->cfg_pkgs[i].cnst_info.sensor_id);
            continue;
        }
        ...
        if (addr && cfg_bin->cfg_pkgs[i].cnst_info.fw_mask[0]) {
            ...
            if (r < 0) {
                ts_err("failed read fw_mask pkg:%d, reg:0x%02x", i, addr);
                goto get_default_pkg;
            }
            if (strncmp(temp_fw_mask, cfg_bin->cfg_pkgs[i].cnst_info.fw_mask, sizeof(temp_fw_mask))) {
                ts_err("pkg:%d, fw_mask contrast FAILED, reg:0x%02x,", i, addr);
                ts_err("mask from i2c:%s, mask of cfg bin:%s", temp_fw_mask, cfg_bin->cfg_pkgs[i].cnst_info.fw_mask);
                continue;
            }
        }
        ...
        if (strncmp(temp_pid, cfg_bin->cfg_pkgs[i].cnst_info.hw_pid, read_len)) {
            ...
            ts_err("pid from i2c:%s, pid of cfg bin:%s", temp_pid, cfg_bin->cfg_pkgs[i].cnst_info.hw_pid);
            continue;
        }

        ts_info("try get package info: ic type %s, cfg type %d", cfg_bin->cfg_pkgs[i].cnst_info.ic_type, cfg_bin->cfg_pkgs[i].cnst_info.cfg_type);
...
static int goodix_read_cfg_bin(struct device *dev, struct goodix_cfg_bin *cfg_bin)
{
    ...
    for (i = 0; i < TS_RQST_FW_RETRY_TIMES; i++) {
        ...
        if (r < 0) {
            ts_err("failed get cfg bin[%s] error:%d, try_times:%d", cfg_bin_name, r, i + 1);
            msleep(1000);
        } else {
...
static int goodix_later_init_thread(void *data)
{
    ...
    if (ts_dev->cfg_bin_state == CFG_BIN_STATE_TEMP) {
        ...
        ret = goodix_do_fw_update(UPDATE_MODE_BLOCK | UPDATE_MODE_FORCE | UPDATE_MODE_FLASH_CFG | UPDATE_MODE_SRC_REQUEST);
        if (ret) {
        ...
        ret = goodix_do_fw_update(UPDATE_MODE_BLOCK | UPDATE_MODE_FLASH_CFG | UPDATE_MODE_SRC_REQUEST);
        if (ret) {
...
int goodix_start_later_init(struct goodix_ts_core *ts_core)
{
    ...
    init_thrd = kthread_run(goodix_later_init_thread, ts_core, "goodix_init_thread");
