		...
		struct device_node *core_node, const char *name)
{
	...
	printk("%s: Touch setting:'%s' size:%d\n", __func__, name, size);
		...
		struct device_node *core_node)
{
	...
	pdata->irq_gpio = of_get_named_gpio_flags(core_node, "cy,irq_gpio", 0, (enum of_gpio_flags *)&irq_flags);
	if( !gpio_is_valid(pdata->irq_gpio)) {
		printk("cyttsp5:irq_gpio %d in Invalid!\n" , pdata->irq_gpio);
		goto fail_free;
	}
	//	pdata->irq_gpio= gpio_to_irq(pdata->irq_gpio);
		
	//	if (gpio_request(pdata->irq_gpio, "cyttsp5-irq") != 0) {
	//			printk("cyttsp5:gpio_request irq_gpio error\n");
	//			goto fail_free;
	//		}
#endif
...
#else
			pdata->power_gpio = of_get_named_gpio_flags(core_node, "cy,power_gpio", 0, &power_flags);
			if( !gpio_is_valid(pdata->power_gpio) ){
				//if (gpio_request(pdata->rst_gpio, "cyttsp5-reset") != 0) {
				//	 gpio_free(pdata->rst_gpio);
					 printk("cyttsp5:gpio_request no power_gpio\n");
					 pdata->power_gpio = INVALID_GPIO; // 20210728: platform.c 里面判断的是这个值。
					 //goto fail_free;
				//}
	...
	if (gpio_request(value, NULL) != 0)
		pdata->rst_gpio = value;
#else
	pdata->rst_gpio = of_get_named_gpio_flags(core_node, "cy,rst_gpio", 0, &rst_flags);
	if( !gpio_is_valid(pdata->rst_gpio) ){
		//if (gpio_request(pdata->rst_gpio, "cyttsp5-reset") != 0) {
		//	 gpio_free(pdata->rst_gpio);
		//	 printk("cyttsp5:gpio_request reset_gpio error\n");
			 goto fail_free;
		//}
		//gpio_direction_output(pdata->rst_gpio, 1);
	}
	...
	for (i = 0; (unsigned int)i < ARRAY_SIZE(touch_setting_names); i++) {
		...
		if (IS_ERR(pdata->sett[i])) {
			...
			printk("%s: No data for setting '%s'\n", __func__,
				touch_setting_names[i]);
	}

	printk("%s: power_gpio:%d irq_gpio:%d rst_gpio:%d\n"
		"hid_desc_register:%d level_irq_udelay:%d vendor_id:%d product_id:%d\n"
		"flags:%d easy_wakeup_gesture:%d\n", __func__,
		pdata->power_gpio, pdata->irq_gpio, pdata->rst_gpio,
...
int cyttsp5_devtree_create_and_get_pdata(struct device *adap_dev)
{
	...
	for_each_child_of_node(adap_dev->of_node, core_node) {
		...
		if (!rc){
			pr_debug("%s: name:%s\n", __func__, name);
			printk("cyttsp5_devtree_create_and_get_pdata: name:%s\n",name);
			}
