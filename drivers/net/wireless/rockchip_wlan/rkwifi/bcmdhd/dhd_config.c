...
extern char wifi_chip_type_string_ht[];
...
const cihp_name_map_t chip_name_map [] = {
	...
	#ifdef CONFIG_CYW_HT
	{BCM43430_CHIP_ID,	1,	DONT_CARE,	FALSE,	"cyw43438",		"azw372"},
	#else
...
#ifdef CONFIG_CYW_HT
	{BCM4345_CHIP_ID,	6,	DONT_CARE,	FALSE,	"cyw43455",		"bcm943457"},
#else	
...
dhd_conf_set_fw_name_by_chip(dhd_pub_t *dhd, char *fw_path)
{
	...
	for (i = 0;  i < sizeof(chip_name_map)/sizeof(chip_name_map[0]);  i++) {
		...
		if (row->chip == chip && row->chiprev == chiprev &&
			...
			strcpy(wifi_chip_type_string_ht,row->module_name);
            printk("wifi_chip_type_string_ht:%s \n",wifi_chip_type_string_ht);
#if 0//def CONFIG_CYW_HT
			strcpy(name_ptr, "");
